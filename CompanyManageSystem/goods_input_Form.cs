    ...
    public partial class goods_input_Form : Form
    {
       ...
       */
        private DataTable deletesaleproduct(DataTable saleproduct, DataTable dtUpdate, String salerecepit)
        {
            ...
            if (flag == false)
            {
                MessageBox.Show("表格中存在不合法的数据,进货编号或者进货编号在进货产品表中已存在!\n" +
                    "如果有合法的数据，该部分合法数据会存储到数据库里。");
            }
            return dtUpdate;
        }
        ...
        {
            String strSql = "select * from in_product";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (dbUpdate())//调用dbUpdate方法提交进货表单
            {
        ...
        private void textBox3_Leave(object sender, EventArgs e)
        {
            if (textBox3.Text.ToString() != "")
            {
                ...
                {
                    MessageBox.Show("请注意，您输入的实际付款金额大于应付金额！！");
