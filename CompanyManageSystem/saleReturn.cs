...
namespace CompanyManageSystem
{
    public partial class saleReturn : Form
    {
       ...
       */
        ...
        {
            String sql = "select * from sout_product";
            MySqlDataAdapter sda = new MySqlDataAdapter(sql, conn);
            DataTable ds = new DataTable();
            sda.Fill(ds);
            //MessageBox.Show(ds.Rows[0][0].ToString()+ ds.Rows[0][1].ToString()+ ds.Rows[0][2].ToString());
        ...
        private DataTable deleteReturnproduct(DataTable product, DataTable dtUpdate)
        {
            ...
            if (flag == 0)
            {
                MessageBox.Show("表格中存在不合法的数据,退回产品在库存中不存在，货物单位不正确!\n" +
      ...
      */
        ...
        {
            String strSql = "select * from sout_product";
        ...
        */
        private void textBox8_Leave(object sender, EventArgs e)
        {
            if (textBox8.Text.ToString() != "")
            {
                if (Convert.ToDouble(textBox8.Text.ToString()) < Convert.ToDouble(textBox7.Text.ToString()))
                {
                    MessageBox.Show("请注意，您输入的实际收款金额小于应收金额！！");
                    textBox9.Text = (Convert.ToDouble(textBox7.Text.ToString()) -
                Convert.ToDouble(textBox8.Text.ToString())).ToString();
                }
                else if (Convert.ToDouble(textBox8.Text.ToString()) < Convert.ToDouble(textBox7.Text.ToString()))
                {
            ...
            {
                textBox9.Text = (Convert.ToDouble(textBox7.Text.ToString()) - 0).ToString();
            }
        }
    }
}
